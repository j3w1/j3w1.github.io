...
/* The consumer-owned bridge from the site's historical variable names to the
   ...
   j3w1/theme references/j3w1-web/mapping.json at the pinned revision, with one
   correction: mapping.json points the sixteen ANSI slots at primitives, which
   the theme marks inspection-only, so they map to the eligible
   color.terminal.ansi.* roles instead (the same heritage values, byte for
   byte). */
...
export const buildThemeCss = (vendorCss, lock) => {
  ...
  const aliases = LEGACY_THEME_MAP.map(([legacy, token]) => {
    ...
    invariant(!token.startsWith("--color-primitive-"), legacy + " maps to a primitive, not a role");
  ...
  return [
    ...
    ":root {",
    "  /* legacy aliases (references/j3w1-web/mapping.json; ANSI slots via terminal roles) */",
    ...aliases,
