...
The public site and `/admin/` client remain dependency-free static GitHub Pages assets. `content/**` is authoritative; `assets/data/content-index.json`, the prerendered entry and collection pages, `sitemap.xml` and `feed.xml` are deterministic public output generated from it by `src/site-pages.js` and `src/generate.js`; the site and j3w1ctl preview consume the same restricted-AST renderer, and `src/html-renderer.js` mirrors it for the pages. A browser publish commits the regenerated pages in the same expected-head commit as the index, sending only files whose bytes changed and deleting pages whose entry is gone. **After changing the generator, redeploy the function before publishing from the browser** — an older generator on Vercel would produce different bytes than `npm run generate`; CI's drift check catches it, and the fix is one local `npm run generate` commit. Raw HTML is disabled and CMS-controlled strings are inserted as text.
